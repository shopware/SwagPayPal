...
class ExpressShippingCallbackService
{
    ...
    public function recalculateCart(
        ...
        if ($cart->getErrors()->filterInstance(ShippingMethodBlockedError::class)->first()
            && ($firstAvailable = $shippingOptions->first()) !== null
            && $firstAvailable->getId() !== $salesChannelContext->getShippingMethod()->getId()
        ) {
            $this->logger->debug('Shipping callback: selected shipping method blocked, switching to first available', ['shippingMethodId' => $firstAvailable->getId()]);
            $salesChannelContext = $this->switchShippingMethod($firstAvailable->getId(), $salesChannelContext);
            $cart = $this->cartService->getCart($salesChannelContext->getToken(), $salesChannelContext, false, true);
            $shippingOptions = $this->shippingOptionsProvider->getShippingOptions($cart, $salesChannelContext);
        }
    ...
    {
        $params = [SalesChannelContextService::SHIPPING_METHOD_ID => $shippingMethodId];
        $token = $this->contextSwitchRoute->switchContext(new RequestDataBag($params), $salesChannelContext)->getToken();
