...
class GooglePayOrderBuilder extends AbstractOrderBuilder
{
    protected function buildPaymentSource(
        ...
        $googlePay->setExperienceContext($this->createExperienceContext($order, $salesChannel, $context, $paymentTransaction, new RequestDataBag($request->request->all())));
    ...
    protected function buildPaymentSourceFromCart(Cart $cart, SalesChannelContext $salesChannelContext, RequestDataBag $requestDataBag, PaymentSource $paymentSource): void
    {
        $googlePay = new GooglePay();
        /** @phpstan-ignore method.deprecated */
        $googlePay->setExperienceContext($this->createExperienceContext($cart, $salesChannelContext->getSalesChannel(), $salesChannelContext->getContext(), null, $requestDataBag));
