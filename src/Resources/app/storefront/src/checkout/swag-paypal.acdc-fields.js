...
export default class SwagPaypalAcdcFields extends SwagPaypalAbstractStandalone {
    ...
    async render(paypal) {
        ...
        this.paymentToken = authenticationResult.profileData.card.id;
    ...
    async onFastlaneSubmit(fastlane, component, event) {
        ...
        try {
            ...
            if (!this.paymentToken) {
                this.paymentToken = (await component.getPaymentToken({billingAddress: this.options.billingAddress})).id;
            }

            const input = document.createElement('input');
            input.setAttribute('type', 'hidden');
            input.setAttribute('name', 'fastlaneToken');
            input.setAttribute('value', this.paymentToken);
