...
use Swag\PayPal\AgenticCommerce\Struct\V1\Builder\ValidationIssueBuilder;
use Swag\PayPal\AgenticCommerce\Struct\V1\Context\InventoryIssueContext;
use Swag\PayPal\AgenticCommerce\Struct\V1\Context\PricingErrorContext;
...
class ValidationIssues
{
    ...
    public function outOfStock(LineItem $item, ?ProductEntity $restockProduct, CurrencyEntity $currency): ValidationIssue
    {
        ...
        $builder
            ...
            ->withUserMessage($this->translator->trans('paypal.agenticCommerce.validationIssue.userMessage.outOfStock', ['%name%' => $item->getLabel(), '%count%' => $stock]))
            ->withItemId($item->getReferencedId() ?? '')
            ->addResolutionOption()
                ->withAction(ResolutionOption::ACTION__REMOVE_ITEM)
                ->withLabel($this->translator->trans('paypal.agenticCommerce.validationIssue.resolutionOption.removeLabel'))
                ->withMetadata()
        ...
        $wait = $builder->addResolutionOption()
            ...
            ->withLabel($this->translator->trans('paypal.agenticCommerce.validationIssue.resolutionOption.waitRestockLabel', ['%name%' => $item->getLabel()]));

        if ($restockProduct) {
            $wait->withMetadata()
                ->withEstimatedTime($this->translator->trans('paypal.agenticCommerce.validationIssue.resolutionOption.estimatedTime', ['%days%' => $restockProduct->getRestockTime()]))
    ...
    public function changedPrice(LineItem $lineItem, string $initPrice, CurrencyEntity $currency, CashRoundingConfig $roundingConfig): ValidationIssue
    {
        ...
        $builder
            ...
            ->withUserMessage($this->translator->trans('paypal.agenticCommerce.validationIssue.userMessage.priceChanged', ['%name%' => $lineItem->getLabel(), '%oldPrice%' => $initPrice, '%newPrice%' => $unitPrice]))
            ->withItemId($lineItem->getReferencedId() ?? '')
            ->withContext($context);

        $builder->addResolutionOption()
            ->withAction(ResolutionOption::ACTION__ACCEPT_NEW_PRICE)
            ->withLabel($this->translator->trans('paypal.agenticCommerce.validationIssue.resolutionOption.acceptLabel', ['%option%' => $unitPrice]))
            ->withMetadata()
