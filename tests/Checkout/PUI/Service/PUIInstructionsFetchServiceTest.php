...
class PUIInstructionsFetchServiceTest extends TestCase
{
    ...
    private PUIInstructionsFetchService $service;
    ...
    public function testFetchPUIInstructionsPending(): void
    {
        $transaction = $this->createOrderTransaction(GetOrderPUIPending::ID);

        try {
            $this->service->fetchPUIInstructions($transaction, TestDefaults::SALES_CHANNEL, Context::createDefaultContext());
        } catch (PaymentInstructionsNotReadyException $e) {
            $this->assertOrderTransactionState(OrderTransactionStates::STATE_IN_PROGRESS, $transaction->getId(), Context::createDefaultContext());
        }
    }

    public function testFetchPUIInstructionsApproved(): void
    {
        $transaction = $this->createOrderTransaction(GetOrderPUIApproved::ID);

        try {
            $this->service->fetchPUIInstructions($transaction, TestDefaults::SALES_CHANNEL, Context::createDefaultContext());
        } catch (PaymentInstructionsNotReadyException $e) {
            $this->assertOrderTransactionState(OrderTransactionStates::STATE_AUTHORIZED, $transaction->getId(), Context::createDefaultContext());
        }
    }

    public function testFetchPUIInstructionsDuplicateApproved(): void
    {
        $transaction = $this->createOrderTransaction(GetOrderPUIApproved::ID);
    ...
    public function testFetchPUIInstructionsCompleted(): void
    {
        $transaction = $this->createOrderTransaction(GetOrderPUICompleted::ID);

        $response = $this->service->fetchPUIInstructions($transaction, TestDefaults::SALES_CHANNEL, Context::createDefaultContext());
        $this->assertOrderTransactionState(OrderTransactionStates::STATE_PAID, $transaction->getId(), Context::createDefaultContext());
    ...
    public function testFetchPUIInstructionsExisting(): void
    {
        $transactionId = $this->createOrderTransaction(GetOrderPUIPending::ID, ['deposit_bank_details' => ['iban' => GetOrderPUICompleted::BANK_IBAN]]);

        $response = $this->service->fetchPUIInstructions($transactionId, TestDefaults::SALES_CHANNEL, Context::createDefaultContext());
        static::assertSame(GetOrderPUICompleted::BANK_IBAN, $response->getDepositBankDetails()->getIban());
    }

    public function testFetchPUIInstructionsInvalidOrder(): void
    {
        $transaction = $this->createOrderTransaction(GetRefundedOrderCapture::ID);
    ...
    public function testFetchPUIInstructionsVoided(): void
    {
        $transaction = $this->createOrderTransaction(GetOrderPUIVoided::ID);

        try {
            $this->service->fetchPUIInstructions($transaction, TestDefaults::SALES_CHANNEL, Context::createDefaultContext());
        } catch (PaymentInstructionsNotReadyException $e) {
            $this->assertOrderTransactionState(OrderTransactionStates::STATE_FAILED, $transaction->getId(), Context::createDefaultContext());
        }
    }

    private function createOrderTransaction(string $paypalOrderId, ?array $instructions = null): OrderTransactionEntity
    {
